...
function App() {
  ...
  useEffect(() => {
    if (userData?.id && userData?.rol==='dueño') {
      try { setGastosD(JSON.parse(localStorage.getItem('gastos_d_'+userData.id)||'[]')) } catch {}
    }
  }, [userData?.id])
  useEffect(() => {
    fetch(`${API}/api/anuncios`).then(r=>r.json()).then(d=>{ if(d.success && d.data?.length) setAdBanners(d.data.filter((a:any) => a.activo)) }).catch(()=>{})
  ...
  if (loggedIn && userData?.rol==='barbero') {
    ...
    const isPendingB = localStorage.getItem('pro_b_'+userData?.id)==='1'
    const nowB = new Date()
    const citasComp = citas.filter((c:any)=>c.estado==='completada'||c.estado==='agendada')
    const ingrHoyB = citasComp.filter((c:any)=>{const d=new Date(c.fecha);return d.toDateString()===nowB.toDateString()}).reduce((a:number,c:any)=>a+(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0),0)
    const ingrSemB = citasComp.filter((c:any)=>{const d=new Date(c.fecha);return nowB.getTime()-d.getTime()<7*86400000&&d<=nowB}).reduce((a:number,c:any)=>a+(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0),0)
    const ingrMesB = citasComp.filter((c:any)=>{const d=new Date(c.fecha);return d.getMonth()===nowB.getMonth()&&d.getFullYear()===nowB.getFullYear()}).reduce((a:number,c:any)=>a+(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0),0)
    return (
      <div className="dashboard-container">
        <nav className="navbar">
              ...
              {isProB ? (
                <>
                  <div style={{textAlign:'center',marginBottom:24}}>
                    ...
                    <p style={{fontSize:11,color:'#C9A84C',textTransform:'uppercase',letterSpacing:4,marginBottom:4}}>Plan Pro Activo</p>
                    <h2 style={{fontSize:22,fontWeight:900,margin:'0 0 4px'}}>Tu Panel Profesional</h2>
                    <p style={{color:'#555',fontSize:12,marginTop:4}}>{new Date().toLocaleDateString('es-ES',{weekday:'long',day:'2-digit',month:'long'})}</p>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:20}}>
                    {[{l:'Hoy',v:`$${ingrHoyB.toFixed(0)}`,c:'#C9A84C'},{l:'Semana',v:`$${ingrSemB.toFixed(0)}`,c:'#00D4FF'},{l:'Este mes',v:`$${ingrMesB.toFixed(0)}`,c:'#2ECC71'},{l:'Total citas',v:citasComp.length,c:'#BB8FCE'}].map(s=>(
                      <div key={s.l} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:'18px 14px',textAlign:'center'}}>
                        <p style={{fontSize:26,fontWeight:900,color:s.c as string,margin:0}}>{s.v}</p>
                        <p style={{fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:1,marginTop:4}}>{s.l}</p>
                      </div>
                    ))}
                  </div>
                  {citas.length>0&&<div style={{marginBottom:20}}>
                    <p style={{fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:2,fontWeight:700,marginBottom:12}}>Últimas citas</p>
                    {citas.slice(0,5).map((c:any)=>(
                      <div key={c.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:10,padding:'10px 14px',marginBottom:6}}>
                        <div>
                          <p style={{margin:0,fontWeight:700,fontSize:13}}>{c.cliente?.nombre||c.cliente_nombre||'Cliente'}</p>
                          <p style={{margin:'2px 0 0',fontSize:11,color:'#555'}}>{c.servicio?.nombre||c.servicio_nombre||'Servicio'} · {c.fecha} {c.hora}</p>
                        </div>
                        <span style={{fontWeight:800,color:'#C9A84C',fontSize:14}}>${(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0).toFixed(0)}</span>
                      </div>
                    ))}
                  </div>}
                  {perfilBarbero?.calificacion&&<div style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.2)',borderRadius:12,padding:'16px',textAlign:'center'}}>
                    <p style={{margin:0,fontSize:11,color:'#C9A84C',textTransform:'uppercase',letterSpacing:2,fontWeight:700}}>Tu calificación promedio</p>
                    <p style={{margin:'6px 0 0',fontSize:32,fontWeight:900,color:'#C9A84C'}}>⭐ {Number(perfilBarbero.calificacion).toFixed(1)}</p>
                  </div>}
                </>
              ) : (
                <>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:24}}>
                    {[{icon:'📊',t:'Ingresos en tiempo real',d:'Hoy, semana y mes'},{icon:'🌟',t:'Perfil destacado',d:'Apareces primero en búsquedas'},{icon:'⭐',t:'Tus reseñas',d:'Ve y gestiona tus calificaciones'},{icon:'📋',t:'Exportar reporte',d:'Historial listo para imprimir'},{icon:'🔔',t:'Recordatorios',d:'Aviso a clientes 24h antes'},{icon:'🎖️',t:'Badge Pro',d:'Sello visible en tu perfil'}].map(f=>(
                  ...
                  <div style={{background:'rgba(201,168,76,0.03)',border:'1px solid rgba(201,168,76,0.25)',borderRadius:16,padding:24,marginBottom:16}}>
                    <div style={{textAlign:'center',marginBottom:16}}>
                      <p style={{fontSize:36,fontWeight:900,color:'#C9A84C',margin:'0 0 4px'}}>$3.99<span style={{fontSize:14,fontWeight:400,color:'#666'}}> USD/mes</span></p>
                      <p style={{fontSize:12,color:'#555',marginBottom:14}}>Pago mensual · Cancela cuando quieras</p>
                      <div style={{background:'rgba(0,0,0,0.35)',borderRadius:12,padding:'14px',marginBottom:4}}>
                        <p style={{fontSize:9,color:'#444',textTransform:'uppercase',letterSpacing:3,marginBottom:2}}>Binance Pay ID</p>
                        <p style={{fontSize:26,fontWeight:900,letterSpacing:5,color:'#C9A84C',margin:'0 0 4px'}}>176779028</p>
                        <p style={{fontSize:10,color:'#555'}}>Envía $3.99 USDT/mes y pulsa el botón</p>
                      </div>
                    </div>
                    {isPendingB ? (
                      <div style={{textAlign:'center'}}>
                        <p style={{color:'#FFA500',fontSize:13,fontWeight:700,margin:'0 0 14px'}}>⏳ Solicitud enviada — en revisión</p>
                        <button onClick={async()=>{
                          try {
                            const r=await fetch(`${API}/api/anuncios`)
                            const d=await r.json()
                            const email=userData?.email||''
                            const activo=(d.data||[]).find((a:any)=>a.titulo?.startsWith('💎 PLAN PRO - Barbero')&&a.anunciante_email===email&&a.activo===true)
                            if(activo){localStorage.setItem('pro_activo_b_'+userData?.id,'1');window.location.reload()}
                            else{alert('⏳ Tu Plan Pro aún está en revisión. Te avisaremos cuando esté listo.')}
                          } catch{alert('Error de conexión. Intenta de nuevo.')}
                        }} style={{background:'rgba(255,165,0,0.1)',border:'1px solid rgba(255,165,0,0.35)',color:'#FFA500',borderRadius:10,padding:'11px 24px',fontSize:13,fontWeight:700,cursor:'pointer',width:'100%'}}>
                      ...
                      <button onClick={async(e)=>{
                        ...
                        const nombre=userData?.nombre||perfilBarbero?.nombre||'Barbero'
                        const email=userData?.email||''
                        const tel=userData?.telefono||perfilBarbero?.telefono||'N/A'
                        const ciudad=userData?.ciudad||perfilBarbero?.ciudad||userData?.pais||'N/A'
                        if(!email){alert('Tu cuenta no tiene email. Contacta soporte.');btn.disabled=false;btn.textContent='💎 Ya pagué — Activar Pro';return}
                        const payload={titulo:'💎 PLAN PRO - Barbero',subtitulo:nombre,anunciante_nombre:nombre,anunciante_email:email,anunciante_telefono:tel,ciudad,pais:userData?.pais||'Venezuela',boton_texto:'',boton_url:'',imagen_url:'https://cutconnect-app.vercel.app/pro',latitud:'',longitud:'',activo:false,estado:'pendiente'}
                        try {
                          ...
                          const data=await res.json()
                          if(res.ok||data.success||data.id||data.message){localStorage.setItem('pro_b_'+userData?.id,'1');window.location.reload()}
                          else{alert('Error: '+(data.error||data.message||'Intenta de nuevo'));btn.disabled=false;btn.textContent='💎 Ya pagué — Activar Pro'}
                        } catch{alert('Error de conexión. Intenta de nuevo.');btn.disabled=false;btn.textContent='💎 Ya pagué — Activar Pro'}
                      }} style={{width:'100%',background:'linear-gradient(135deg,#C9A84C,#B8972A)',color:'#000',border:'none',borderRadius:10,padding:'14px',fontSize:13,fontWeight:800,cursor:'pointer',letterSpacing:0.5,textTransform:'uppercase',boxShadow:'0 4px 16px rgba(201,168,76,0.3)'}}>
                        💎 Ya pagué — Activar Pro
                      </button>
                    )}
                  </div>
                  <a href={`https://wa.me/+32455136804?text=Hola%20CutConnect%2C%20soy%20${encodeURIComponent(userData?.nombre||'barbero')}%20y%20acabo%20de%20pagar%20el%20Plan%20Pro%20Barbero%20por%20Binance%20Pay.`} style={{display:'block',background:'#25D366',color:'#fff',textAlign:'center',padding:'13px',borderRadius:10,fontWeight:700,textDecoration:'none',fontSize:13,textTransform:'uppercase',letterSpacing:1}}>
                    📱 Enviar comprobante por WhatsApp
                  </a>
                </>
              )}
  ...
  if (loggedIn && userData?.rol==='dueño' && ['trial','activo','aprobado'].includes(userData?.estado_verificacion)) {
    ...
    const isPendingD = localStorage.getItem('pro_d_'+userData?.id)==='1'
    const nowD = new Date()
    const citasCompD = citas.filter((c:any)=>c.estado==='completada'||c.estado==='agendada')
    const ingrHoyD = citasCompD.filter((c:any)=>{const d=new Date(c.fecha);return d.toDateString()===nowD.toDateString()}).reduce((a:number,c:any)=>a+(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0),0)
    const ingrSemD = citasCompD.filter((c:any)=>{const d=new Date(c.fecha);return nowD.getTime()-d.getTime()<7*86400000&&d<=nowD}).reduce((a:number,c:any)=>a+(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0),0)
    const ingrMesD = citasCompD.filter((c:any)=>{const d=new Date(c.fecha);return d.getMonth()===nowD.getMonth()&&d.getFullYear()===nowD.getFullYear()}).reduce((a:number,c:any)=>a+(Number(c.precio)||Number(c.servicio_precio)||Number(c.servicio?.precio)||0),0)
    const gastosMesD = gastosD.filter((g:any)=>{const d=new Date(g.fecha);return d.getMonth()===nowD.getMonth()&&d.getFullYear()===nowD.getFullYear()}).reduce((a:number,g:any)=>a+Number(g.monto),0)
    return (
      <div className="dashboard-container">
        <nav className="navbar">
          ...
          <div className="nav-links">
            ...
            <button className={currentPage===(isProD?'dashboard':'pro-dueno')?'active':''} onClick={()=>{const t=isProD?'dashboard':'pro-dueno';setCurrentPage(t);if(isProD){cargarCitasDueno();cargarRanking();cargarMisBarberos()}}} style={{color:'#C9A84C',fontWeight:700}}>💎 Pro{isProD&&<span style={{fontSize:9,verticalAlign:'super'}}>●</span>}</button>
            <button className="btn-logout" onClick={handleLogout}>Salir</button>
          </div>
        </nav>
        <div className="dashboard-content">
              ...
              {isProD ? (
                <>
                  <div style={{marginBottom:16}}>
                    <p style={{fontSize:10,color:'#C9A84C',textTransform:'uppercase',letterSpacing:3,fontWeight:700,marginBottom:12}}>💎 Pro · Ingresos del negocio</p>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                      {[{l:'Hoy',v:`$${ingrHoyD.toFixed(0)}`,c:'#C9A84C'},{l:'Esta semana',v:`$${ingrSemD.toFixed(0)}`,c:'#00D4FF'},{l:'Este mes',v:`$${ingrMesD.toFixed(0)}`,c:'#2ECC71'},{l:'Citas activas',v:citasCompD.length,c:'#BB8FCE'}].map(s=>(
                        <div key={s.l} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:'16px 12px',textAlign:'center'}}>
                          <p style={{fontSize:24,fontWeight:900,color:s.c as string,margin:0}}>{s.v}</p>
                          <p style={{fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:1,marginTop:4}}>{s.l}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:16,padding:20,marginBottom:16}}>
                    <p style={{fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:2,fontWeight:700,marginBottom:10}}>💰 Gastos — {new Date().toLocaleDateString('es-ES',{month:'long',year:'numeric'})}</p>
                    <div style={{display:'flex',gap:16,flexWrap:'wrap',marginBottom:14}}>
                      <span style={{fontSize:12,color:'#2ECC71'}}>Ingresos: <strong>${ingrMesD.toFixed(0)}</strong></span>
                      <span style={{fontSize:12,color:'#FF6B6B'}}>Gastos: <strong>${gastosMesD.toFixed(0)}</strong></span>
                      <span style={{fontSize:12,color:ingrMesD-gastosMesD>=0?'#C9A84C':'#FF6B6B',fontWeight:700}}>Ganancia: ${(ingrMesD-gastosMesD).toFixed(0)}</span>
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 80px 110px 36px',gap:6,marginBottom:8,alignItems:'center'}}>
                      <input placeholder="Descripción" value={gastosFormD.descripcion} onChange={e=>setGastosFormD({...gastosFormD,descripcion:e.target.value})} style={{padding:'7px 10px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:8,color:'#fff',fontSize:12}} />
                      <input type="number" placeholder="$0" min="0" value={gastosFormD.monto} onChange={e=>setGastosFormD({...gastosFormD,monto:e.target.value})} style={{padding:'7px 8px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:8,color:'#fff',fontSize:12}} />
                      <select value={gastosFormD.categoria} onChange={e=>setGastosFormD({...gastosFormD,categoria:e.target.value})} style={{padding:'7px 6px',background:'rgba(20,20,20,0.95)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:8,color:'#fff',fontSize:11}}>
                        {['Suministros','Nómina','Alquiler','Marketing','Servicios','Equipos','Otro'].map(c=><option key={c}>{c}</option>)}
                      </select>
                      <button onClick={()=>{
                        if(!gastosFormD.descripcion||!gastosFormD.monto)return
                        const g={id:Date.now(),descripcion:gastosFormD.descripcion,monto:Number(gastosFormD.monto),categoria:gastosFormD.categoria,fecha:new Date().toISOString().split('T')[0]}
                        const upd=[...gastosD,g];setGastosD(upd);localStorage.setItem('gastos_d_'+userData?.id,JSON.stringify(upd))
                        setGastosFormD({descripcion:'',monto:'',categoria:'Suministros'})
                      }} style={{background:'linear-gradient(135deg,#C9A84C,#B8972A)',color:'#000',border:'none',borderRadius:8,fontWeight:700,cursor:'pointer',fontSize:16,height:36}}>+</button>
                    </div>
                    <div style={{maxHeight:150,overflowY:'auto'}}>
                      {gastosD.filter((g:any)=>{const d=new Date(g.fecha);return d.getMonth()===nowD.getMonth()&&d.getFullYear()===nowD.getFullYear()}).slice().reverse().map((g:any)=>(
                        <div key={g.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 10px',background:'rgba(255,107,107,0.04)',borderRadius:7,marginBottom:4,border:'1px solid rgba(255,107,107,0.1)'}}>
                          <div style={{minWidth:0,flex:1}}>
                            <span style={{fontSize:12,color:'#ddd',fontWeight:600}}>{g.descripcion}</span>
                            <span style={{fontSize:10,color:'#444',marginLeft:8}}>{g.categoria}</span>
                          </div>
                          <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
                            <span style={{fontSize:12,fontWeight:800,color:'#FF6B6B'}}>-${Number(g.monto).toFixed(0)}</span>
                            <button onClick={()=>{const upd=gastosD.filter((x:any)=>x.id!==g.id);setGastosD(upd);localStorage.setItem('gastos_d_'+userData?.id,JSON.stringify(upd))}} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:13,padding:0,lineHeight:1}}>✕</button>
                          </div>
                        </div>
                      ))}
                      {gastosD.filter((g:any)=>{const d=new Date(g.fecha);return d.getMonth()===nowD.getMonth()&&d.getFullYear()===nowD.getFullYear()}).length===0&&<p style={{textAlign:'center',color:'#333',fontSize:12,padding:'8px 0'}}>Sin gastos registrados este mes</p>}
                    </div>
                  </div>
                  {rankingBarberos.length>0&&<div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:16,padding:20,marginBottom:16}}>
                    <p style={{fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:2,fontWeight:700,marginBottom:14}}>🏆 Ranking del equipo · este mes</p>
                    {rankingBarberos.slice(0,5).map((b:any,i:number)=>(
                      <div key={b.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                        <span style={{fontSize:18,width:26,textAlign:'center',flexShrink:0}}>{i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}.`}</span>
                        <div style={{flex:1,minWidth:0}}>
                          <p style={{margin:0,fontWeight:700,fontSize:13,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{b.nombre||'Barbero'}</p>
                          <div style={{height:3,background:'rgba(255,255,255,0.06)',borderRadius:2,marginTop:5}}>
                            <div style={{height:'100%',borderRadius:2,background:i===0?'#C9A84C':i===1?'#aaa':'#cd7f32',width:`${((b.total_citas||0)/maxCitas)*100}%`,transition:'width 0.5s'}} />
                          </div>
                        </div>
                        <span style={{fontSize:12,color:'#C9A84C',fontWeight:700,flexShrink:0}}>{b.total_citas||0} citas</span>
                      </div>
                    ))}
                  </div>}
                  <button onClick={()=>{
                    const rows=[['Fecha','Barbero','Cliente','Servicio','Precio','Estado'],...citas.map((c:any)=>[c.fecha,c.barbero?.nombre||'',c.cliente?.nombre||c.cliente_nombre||'',c.servicio?.nombre||c.servicio_nombre||'',c.precio||c.servicio_precio||'0',c.estado])]
                    const csv=rows.map(r=>r.map((x:any)=>String(x).replace(/,/g,';')).join(',')).join('\n')
                    const a=document.createElement('a');a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv);a.download=`citas_${new Date().toISOString().split('T')[0]}.csv`;a.click()
                  }} style={{width:'100%',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'#fff',borderRadius:10,padding:'10px',fontSize:12,fontWeight:700,cursor:'pointer',marginBottom:4}}>
            ...
            <div className="page" style={{background:'linear-gradient(180deg,rgba(201,168,76,0.04) 0%,transparent 300px)'}}>
              {isProD ? (
                <div style={{textAlign:'center',padding:'40px 24px'}}>
                  ...
                  <h2 style={{fontSize:22,fontWeight:900,marginBottom:8}}>Plan Pro Activo</h2>
                  <p style={{color:'#888',fontSize:13,lineHeight:1.7,marginBottom:28}}>Tu plan está activo. Accede a las funciones Pro desde tu Dashboard.</p>
                  <button onClick={()=>{setCurrentPage('dashboard');cargarCitasDueno();cargarRanking();cargarMisBarberos()}} style={{background:'linear-gradient(135deg,#C9A84C,#B8972A)',color:'#000',border:'none',borderRadius:14,padding:'16px 36px',fontSize:14,fontWeight:800,cursor:'pointer'}}>
                    Ir al Dashboard Pro →
                  </button>
                </div>
              ) : isPendingD ? (
                <div style={{textAlign:'center',padding:'40px 24px'}}>
                  ...
                  <h2 style={{fontSize:20,fontWeight:800,marginBottom:8}}>Solicitud en revisión</h2>
                  <p style={{color:'#888',fontSize:13,lineHeight:1.7,marginBottom:24}}>Recibimos tu pago. Tu Plan Pro se activará en menos de 24 horas.</p>
                  <button onClick={async()=>{
                    try {
                      const r=await fetch(`${API}/api/anuncios`)
                      const d=await r.json()
                      const email=userData?.email||''
                      const activo=(d.data||[]).find((a:any)=>a.titulo?.startsWith('💎 PLAN PRO - Negocio')&&a.anunciante_email===email&&a.activo===true)
                      if(activo){localStorage.setItem('pro_activo_d_'+userData?.id,'1');window.location.reload()}
                      else{alert('⏳ Aún en revisión. Te notificaremos por WhatsApp en cuanto esté activo.')}
                    } catch{alert('Error de conexión. Intenta de nuevo.')}
                  }} style={{background:'rgba(255,165,0,0.1)',border:'1px solid rgba(255,165,0,0.35)',color:'#FFA500',borderRadius:10,padding:'12px 28px',fontSize:13,fontWeight:700,cursor:'pointer',display:'block',margin:'0 auto 16px'}}>
                    🔄 Verificar activación
                  </button>
                  <button onClick={()=>{localStorage.removeItem('pro_d_'+userData?.id);window.location.reload()}} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:11,textDecoration:'underline'}}>¿No has pagado aún? Reintentar</button>
                </div>
              ) : (
                <>
                  <div style={{textAlign:'center',marginBottom:28}}>
                    ...
                    <h2 style={{fontSize:24,fontWeight:900,marginBottom:8}}>Gestión profesional de tu barbería</h2>
                    <p style={{color:'#666',fontSize:13,lineHeight:1.7}}>Todo lo que necesitas para llevar tu negocio al siguiente nivel</p>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:24}}>
                    {[{icon:'📊',t:'Ingresos en tiempo real',d:'Hoy, semana y mes'},{icon:'💰',t:'Control de gastos',d:'Gastos vs ingresos = ganancia neta'},{icon:'🏆',t:'Ranking del equipo',d:'Quién genera más citas'},{icon:'📥',t:'Exportar CSV',d:'Reporte completo de citas'},{icon:'📣',t:'Publicidad prioritaria',d:'Tu barbería primero en búsquedas'},{icon:'🌟',t:'Perfil verificado',d:'Sello Pro visible para clientes'}].map(f=>(
                      <div key={f.t} style={{background:'rgba(201,168,76,0.04)',border:'1px solid rgba(201,168,76,0.1)',borderRadius:12,padding:'14px 12px',textAlign:'center'}}>
                        <div style={{fontSize:24,marginBottom:6}}>{f.icon}</div>
                        <p style={{fontSize:12,fontWeight:700,color:'#fff',marginBottom:3,lineHeight:1.3}}>{f.t}</p>
                        <p style={{fontSize:10,color:'#555',lineHeight:1.4}}>{f.d}</p>
                  ...
                  <div style={{background:'rgba(201,168,76,0.03)',border:'1px solid rgba(201,168,76,0.25)',borderRadius:16,padding:24,marginBottom:16}}>
                    ...
                    <div style={{background:'rgba(0,0,0,0.3)',borderRadius:12,padding:'16px',marginBottom:16,textAlign:'center'}}>
                      <p style={{fontSize:9,color:'#444',textTransform:'uppercase',letterSpacing:3,marginBottom:8}}>Escanea con Binance Pay</p>
                      <img alt="QR Binance Pay" style={{width:160,height:160,borderRadius:10,border:'1px solid rgba(201,168,76,0.3)',display:'block',margin:'0 auto 16px'}} />
                      <p style={{fontSize:9,color:'#444',textTransform:'uppercase',letterSpacing:3,marginTop:10,marginBottom:2}}>ID de pago Binance</p>
                      <p style={{fontSize:24,fontWeight:900,letterSpacing:4,color:'#C9A84C',margin:0}}>176779028</p>
                    </div>
                    <button onClick={async(e)=>{
                      const btn=e.currentTarget;btn.disabled=true;btn.textContent='Enviando...'
                      const nombre=userData?.negocio_nombre||userData?.nombre||'Dueño'
                      const email=userData?.email||''
                      const ciudad=userData?.ciudad||userData?.pais||'N/A'
                      if(!email){alert('Tu cuenta no tiene email. Contacta soporte.');btn.disabled=false;btn.textContent='💎 Ya pagué — Activar Pro';return}
                      const payload={titulo:'💎 PLAN PRO - Negocio',subtitulo:nombre,anunciante_nombre:nombre,anunciante_email:email,anunciante_telefono:userData?.telefono||'N/A',ciudad,pais:userData?.pais||'Venezuela',boton_texto:'',boton_url:'',imagen_url:'https://cutconnect-app.vercel.app/pro',latitud:'',longitud:'',activo:false,estado:'pendiente'}
                      try {
                        const res=await fetch(`${API}/api/anuncios/solicitud`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
                        const data=await res.json()
                        if(res.ok||data.success||data.id||data.message){localStorage.setItem('pro_d_'+userData?.id,'1');window.location.reload()}
                        else{alert('Error: '+(data.error||data.message||'Intenta de nuevo'));btn.disabled=false;btn.textContent='💎 Ya pagué — Activar Pro'}
                      } catch{alert('Error de conexión. Intenta de nuevo.');btn.disabled=false;btn.textContent='💎 Ya pagué — Activar Pro'}
                    }} style={{width:'100%',background:'linear-gradient(135deg,#C9A84C,#B8972A)',color:'#000',border:'none',borderRadius:10,padding:'14px',fontSize:13,fontWeight:800,cursor:'pointer',letterSpacing:0.5,textTransform:'uppercase',boxShadow:'0 4px 16px rgba(201,168,76,0.3)',marginBottom:12}}>
                      💎 Ya pagué — Activar Pro
                    </button>
                  </div>
                  <a href={`https://wa.me/+32455136804?text=Hola%20CutConnect%2C%20soy%20${encodeURIComponent(userData?.negocio_nombre||'dueño')}%20y%20acabo%20de%20pagar%20el%20Plan%20Pro%20Negocio%20por%20Binance%20Pay.`} style={{display:'block',background:'#25D366',color:'#fff',textAlign:'center',padding:'14px',borderRadius:10,fontWeight:700,textDecoration:'none',fontSize:13,textTransform:'uppercase',letterSpacing:1}}>
                    📱 Enviar comprobante por WhatsApp
                  </a>
                </>
              )}
