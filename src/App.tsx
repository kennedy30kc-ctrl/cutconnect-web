...
function App() {
  ...
  useEffect(() => { if (userData?.id && userData?.rol==='due\u00f1o') { try { setGastosD(JSON.parse(localStorage.getItem('gastos_d_'+userData.id)||'[]')) } catch {} } }, [userData?.id])
              ...
              {isProD ? (
                <div style={{background:'linear-gradient(135deg,rgba(201,168,76,0.08),rgba(201,168,76,0.02))',border:'1px solid rgba(201,168,76,0.2)',borderRadius:16,padding:24}}>
                  ...
                  <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:12,padding:16,marginBottom:16}}>
                    ...
                    <button onClick={()=>{if(!gastosFormD.descripcion||!gastosFormD.monto)return;const nuevo={id:Date.now(),descripcion:gastosFormD.descripcion,monto:Number(gastosFormD.monto),categoria:gastosFormD.categoria,fecha:new Date().toISOString().split('T')[0]};const act=[...gastosD,nuevo];setGastosD(act);localStorage.setItem('gastos_d_'+userData?.id,JSON.stringify(act));setGastosFormD({descripcion:'',monto:'',categoria:'Suministros'})}} style={{width:'100%',background:'rgba(201,168,76,0.15)',border:'1px solid #C9A84C',borderRadius:8,padding:'9px',color:'#C9A84C',fontWeight:700,cursor:'pointer',fontSize:12}}>+ Agregar gasto</button>
                  </div>
                  {gastosD.length>0&&(
