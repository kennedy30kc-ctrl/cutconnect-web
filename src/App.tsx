          ...
          {adminPage==='pro'&&(
            <div>
              <div style={{background:'rgba(201,168,76,0.04)',border:'1px solid rgba(201,168,76,0.15)',borderRadius:14,padding:20,marginBottom:20,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div><h3 style={{color:'#C9A84C',fontWeight:800,fontSize:16,marginBottom:4}}>💎 Planes Pro</h3><p style={{color:'#555',fontSize:12,margin:0}}>Activa o niega solicitudes ($3.99/mes)</p></div>
              ...
              <input type="text" placeholder="🔍 Buscar..." value={adminProSearch} onChange={e=>setAdminProSearch(e.target.value)} style={{width:'100%',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'10px 14px',color:'#fff',fontSize:13,outline:'none',boxSizing:'border-box',marginBottom:20}}/>
              <p style={{fontSize:11,color:'#C9A84C',textTransform:'uppercase',letterSpacing:2,fontWeight:700,marginBottom:10}}>👑 Dueños</p>
              {solicitudesPro.filter((s:any)=>s.tipo==='dueno'&&(!adminProSearch||s.nombre?.toLowerCase().includes(adminProSearch.toLowerCase())||s.email?.toLowerCase().includes(adminProSearch.toLowerCase()))).length===0&&<p style={{color:'#333',fontSize:13,textAlign:'center',padding:'10px 0'}}>Ninguno aún</p>}
              {solicitudesPro.filter((s:any)=>s.tipo==='dueno'&&(!adminProSearch||s.nombre?.toLowerCase().includes(adminProSearch.toLowerCase())||s.email?.toLowerCase().includes(adminProSearch.toLowerCase()))).map((s:any)=>(<div key={s.id} style={{background:'#141414',border:'1px solid '+(s.estado==='activo'?'rgba(74,222,128,0.2)':s.estado==='negado'?'rgba(255,100,100,0.15)':'rgba(201,168,76,0.15)'),borderRadius:12,padding:'14px 16px',marginBottom:10,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div style={{flex:1}}><p style={{fontWeight:700,color:'#fff',fontSize:13,marginBottom:2}}>{s.nombre}</p><p style={{fontSize:11,color:'#555',marginBottom:4}}>{s.email}{s.negocio_nombre?' · '+s.negocio_nombre:''}</p><span style={{fontSize:10,background:s.estado==='activo'?'rgba(74,222,128,0.12)':s.estado==='negado'?'rgba(255,100,100,0.1)':'rgba(255,165,0,0.1)',color:s.estado==='activo'?'#4ade80':s.estado==='negado'?'#f87171':'#FFA500',borderRadius:6,padding:'2px 8px',fontWeight:700}}>{s.estado==='activo'?'✅ Activo':s.estado==='negado'?'❌ Negado':'⏳ Pendiente'}</span>{s.fecha_vencimiento&&<span style={{fontSize:10,color:'#444',marginLeft:6}}>Vence: {new Date(s.fecha_vencimiento).toLocaleDateString()}</span>}</div><div style={{display:'flex',gap:6}}><button onClick={async()=>{const v=new Date();v.setDate(v.getDate()+30);await fetch(SUPA_URL+'/rest/v1/solicitudes_pro?id=eq.'+s.id,{method:'PATCH',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+SUPA_KEY,'Content-Type':'application/json'},body:JSON.stringify({estado:'activo',fecha_vencimiento:v.toISOString()})});setAdminMsg('✅ Activado 30 días');setTimeout(()=>setAdminMsg(''),4000);cargarSolicitudesPro()}} style={{background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.3)',color:'#4ade80',borderRadius:8,padding:'8px 12px',fontSize:11,fontWeight:700,cursor:'pointer'}}>✅ Activar</button><button onClick={async()=>{await fetch(SUPA_URL+'/rest/v1/solicitudes_pro?id=eq.'+s.id,{method:'PATCH',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+SUPA_KEY,'Content-Type':'application/json'},body:JSON.stringify({estado:'negado'})});setAdminMsg('❌ Negado');setTimeout(()=>setAdminMsg(''),4000);cargarSolicitudesPro()}} style={{background:'rgba(255,100,100,0.08)',border:'1px solid rgba(255,100,100,0.2)',color:'#f87171',borderRadius:8,padding:'8px 12px',fontSize:11,fontWeight:700,cursor:'pointer'}}>❌ Negar</button></div></div>))}
              <p style={{fontSize:11,color:'#C9A84C',textTransform:'uppercase',letterSpacing:2,fontWeight:700,margin:'20px 0 10px'}}>✂️ Barberos</p>
              {solicitudesPro.filter((s:any)=>s.tipo==='barbero'&&(!adminProSearch||s.nombre?.toLowerCase().includes(adminProSearch.toLowerCase())||s.email?.toLowerCase().includes(adminProSearch.toLowerCase()))).length===0&&<p style={{color:'#333',fontSize:13,textAlign:'center',padding:'10px 0'}}>Ninguno aún</p>}
              {solicitudesPro.filter((s:any)=>s.tipo==='barbero'&&(!adminProSearch||s.nombre?.toLowerCase().includes(adminProSearch.toLowerCase())||s.email?.toLowerCase().includes(adminProSearch.toLowerCase()))).map((s:any)=>(<div key={s.id} style={{background:'#141414',border:'1px solid '+(s.estado==='activo'?'rgba(74,222,128,0.2)':s.estado==='negado'?'rgba(255,100,100,0.15)':'rgba(255,255,255,0.07)'),borderRadius:12,padding:'14px 16px',marginBottom:10,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div style={{flex:1}}><p style={{fontWeight:700,color:'#fff',fontSize:13,marginBottom:2}}>{s.nombre}</p><p style={{fontSize:11,color:'#555',marginBottom:4}}>{s.email}</p><span style={{fontSize:10,background:s.estado==='activo'?'rgba(74,222,128,0.12)':s.estado==='negado'?'rgba(255,100,100,0.1)':'rgba(255,165,0,0.1)',color:s.estado==='activo'?'#4ade80':s.estado==='negado'?'#f87171':'#FFA500',borderRadius:6,padding:'2px 8px',fontWeight:700}}>{s.estado==='activo'?'✅ Activo':s.estado==='negado'?'❌ Negado':'⏳ Pendiente'}</span>{s.fecha_vencimiento&&<span style={{fontSize:10,color:'#444',marginLeft:6}}>Vence: {new Date(s.fecha_vencimiento).toLocaleDateString()}</span>}</div><div style={{display:'flex',gap:6}}><button onClick={async()=>{const v=new Date();v.setDate(v.getDate()+30);await fetch(SUPA_URL+'/rest/v1/solicitudes_pro?id=eq.'+s.id,{method:'PATCH',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+SUPA_KEY,'Content-Type':'application/json'},body:JSON.stringify({estado:'activo',fecha_vencimiento:v.toISOString()})});setAdminMsg('✅ Barbero Pro activado');setTimeout(()=>setAdminMsg(''),4000);cargarSolicitudesPro()}} style={{background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.3)',color:'#4ade80',borderRadius:8,padding:'8px 12px',fontSize:11,fontWeight:700,cursor:'pointer'}}>✅ Activar</button><button onClick={async()=>{await fetch(SUPA_URL+'/rest/v1/solicitudes_pro?id=eq.'+s.id,{method:'PATCH',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+SUPA_KEY,'Content-Type':'application/json'},body:JSON.stringify({estado:'negado'})});setAdminMsg('❌ Negado');setTimeout(()=>setAdminMsg(''),4000);cargarSolicitudesPro()}} style={{background:'rgba(255,100,100,0.08)',border:'1px solid rgba(255,100,100,0.2)',color:'#f87171',borderRadius:8,padding:'8px 12px',fontSize:11,fontWeight:700,cursor:'pointer'}}>❌ Negar</button></div></div>))}
            </div>
          )}
        ...
        <div className="dashboard-content">
          ...
          <div className="page">
            ...
            <div style={{background:'#141414',border:'1px solid rgba(201,168,76,0.3)',borderRadius:16,padding:24,maxWidth:480}}>
              ...
              <button onClick={async()=>{if(!userData?.id)return;try{const r=await fetch(SUPA_URL+'/rest/v1/solicitudes_pro',{method:'POST',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({usuario_id:String(userData.id),nombre:userData.nombre||'',email:userData.email||'',tipo:'barbero',estado:'pendiente'})});alert('✅ Solicitud enviada. El admin la revisará y activará tu plan.')}catch{alert('✅ Enviado. El admin activará tu plan.')}}} style={{display:'block',width:'100%',background:'linear-gradient(135deg,rgba(201,168,76,0.18),rgba(201,168,76,0.06))',border:'1px solid rgba(201,168,76,0.4)',color:'#C9A84C',borderRadius:8,padding:'12px',fontSize:13,fontWeight:700,cursor:'pointer',marginTop:10,letterSpacing:0.5}}>✅ Ya pagué — Registrar mi solicitud</button>
            </div>
          </div>
        </div>
      </div>
          ...
          {currentPage==='pro-dueno' && (
            <div className="page" style={{background:'linear-gradient(180deg,rgba(201,168,76,0.06) 0%,transparent 320px)'}}>
              ...
              <div style={{background:'#141414',border:'1px solid rgba(201,168,76,0.3)',borderRadius:16,padding:24,marginBottom:20}}>
                ...
                <button onClick={async()=>{if(!userData?.id)return;try{const r=await fetch(SUPA_URL+'/rest/v1/solicitudes_pro',{method:'POST',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({usuario_id:String(userData.id),nombre:userData.nombre||'',email:userData.email||'',tipo:'dueno',negocio_nombre:userData.negocio_nombre||'',estado:'pendiente'})});alert('✅ Solicitud enviada. El admin la revisará pronto.')}catch{alert('✅ Enviado.')}}} style={{display:'block',width:'100%',background:'linear-gradient(135deg,rgba(201,168,76,0.18),rgba(201,168,76,0.06))',border:'1px solid rgba(201,168,76,0.4)',color:'#C9A84C',borderRadius:10,padding:'14px',fontSize:13,fontWeight:700,cursor:'pointer',marginTop:10,letterSpacing:0.5}}>✅ Ya pagué — Registrar mi solicitud</button>
              </div>
            </div>
          )}
