      ...
      <div style={{minHeight:'100vh',background:'#050508',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{padding:'16px 28px 0',display:'flex',gap:4,flexWrap:'wrap',borderBottom:'1px solid rgba(255,255,255,0.04)',marginTop:8}}>
          {[
            ...
            {key:'pro',label:'Pro',icon:'💎',badge:solProPendientes.length},
          ].map(tab=>(
            ...
            <div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:10,marginBottom:24}}>
                {[
                  ...
                  <div key={ix} style={{background:'rgba(12,12,22,0.9)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:12,padding:'16px 12px',textAlign:'center'}}>
                    <div style={{fontSize:28,fontWeight:900,color:s.color,lineHeight:1}}>{s.value}</div>
                    <div style={{fontSize:9,color:'#555',marginTop:5,textTransform:'uppercase',letterSpacing:1}}>{s.label}</div>
                  </div>
                ))}
              ...
              {solicitudesPublicidad.filter((s:any)=>typeof s.titulo==='string'&&s.titulo.startsWith('💎 PLAN PRO')).length===0&&(
                <div style={{textAlign:'center',padding:'40px',color:'#333'}}>
                  ...
                  <p style={{fontSize:12,color:'#333'}}>Cuando un barbero o dueño toque "Ya pagué", aparece aquí</p>
                </div>
              )}
              {solicitudesPublicidad.filter((s:any)=>typeof s.titulo==='string'&&s.titulo.startsWith('💎 PLAN PRO')).sort((a:any,b:any)=>a.estado==='pendiente'?-1:1).map((s:any)=>(
                  ...
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    {s.estado==='pendiente'&&(
                      <button className="btn-admin btn-aprobar" onClick={async()=>{
                        ...
                        await fetch(`${API}/api/admin/anuncios/${s.id}`,{method:'PUT',headers:{'Content-Type':'application/json','x-admin-token':'admin_token_cutconnect'},body:JSON.stringify({...s,activo:true,estado:'activo',fecha_vencimiento:fv.toISOString()})})
                        setAdminMsg('✅ Pro activado'); cargarAdminData(); setTimeout(()=>setAdminMsg(''),4000)
                      }}>✅ Activar Pro</button>
                    )}
                    {s.estado==='activo'&&(
                      <button className="btn-admin btn-suspender" onClick={async()=>{
                        await fetch(`${API}/api/admin/anuncios/${s.id}`,{method:'PUT',headers:{'Content-Type':'application/json','x-admin-token':'admin_token_cutconnect'},body:JSON.stringify({...s,activo:false,estado:'inactivo'})})
                        setAdminMsg('Plan Pro desactivado'); cargarAdminData(); setTimeout(()=>setAdminMsg(''),4000)
                      }}>⏸ Desactivar</button>
                    )}
                    {s.estado==='pendiente'&&(
                      <button className="btn-admin btn-rechazar" onClick={async()=>{
                        await fetch(`${API}/api/admin/anuncios/${s.id}`,{method:'PUT',headers:{'Content-Type':'application/json','x-admin-token':'admin_token_cutconnect'},body:JSON.stringify({...s,activo:false,estado:'inactivo'})})
